...
export const BankManagement: React.FC<BankManagementProps> = ({ onSave, currentBank, availableCourses, showToast }) => {
  ...
  const topicOptions = useMemo(() => {
    ...
    return topics.map(code => {
        ...
        const fullTitle = (index >= 0 && selectedCourse.topics?.[index]) ? selectedCourse.topics[index].name : code!;
        return { 
            value: code!, 
  ...
  const getFullTopicDisplay = (q: Partial<Question>) => {
    ...
    const index = match ? parseInt(match[1], 10) - 1 : -1;
    const fullTitle = (index >= 0 && course.topics?.[index]) ? course.topics[index].name : q.topic;
